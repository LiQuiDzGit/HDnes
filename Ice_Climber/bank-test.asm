...
bra_C49E:
    ;------------------------------------------------------------
    ; PRESS A (Start Menu only)
    ;   $008E cycles: 00 -> 01 -> 02 -> 00
    ;------------------------------------------------------------
    LDA ram_btn_press      ; buttons pressed this frame
    AND #con_btn_A         ; A button ($80)
    BEQ bra_C49E_checkStart
        LDA #$80  ; reset demo timer
    STA $004E ; demo time
    INC $008E              ; 00->01->02->03...
    LDA $008E
    CMP #$03               ; reached 03?
    BCC bra_C49E_checkStart
    LDA #$00               ; wrap back to 00
    STA $008E

bra_C49E_checkStart:
    ;------------------------------------------------------------
    ; Existing code: START button
    ;------------------------------------------------------------
    LDA ram_btn_press
    AND #con_btn_Start
    BEQ bra_C4B9_RTS
...
NEA_UpdateAudio:
    ...
    LDA ram_0053_flag  ; check if demo mode
    BNE nea_rts		   ; skip audio if demo
    ;-----------------------------------------------------------
    ; 1) SFX priority: sfx_3 > sfx_2 > sfx_1
    ;    We pick ONE SFX per frame to avoid “3 sounds at once”.
    ;-----------------------------------------------------------
    LDA ram_sfx_3
    BNE have_sfx3
    LDA ram_sfx_2
...
have_sfx:
    ; A = requested SFX track (00+)
    ;CMP nea_last_sfx
    ;BEQ clear_sfx_requests       ; same as last -> don't spam NEA
    ;STA nea_last_sfx
    STA $4106              ; NEA SFX track = A 

clear_sfx_requests:
    ; Clear all SFX requests (matches Ice Climber engine behavior)
    LDA #$00
    STA ram_sfx_3
    STA ram_sfx_2
    STA ram_sfx_1
...
;no_sfx:
    ; No SFX this frame. Still clear, because the original engine does.
    ;LDA #$00
    ;STA ram_sfx_3
    ;STA ram_sfx_2
    ;STA ram_sfx_1
    ...
    ; Optional behavior:
    ; .if you want the same SFX value to be able to retrigger after silence,
    ; you can reset nea_last_sfx here, like this:
    ;LDA #$FF
    ;STA nea_last_sfx
...
do_music:
    ...
    ;CMP #$02 ; main
    ;BEQ bgm_loop
	;LDA #%00000000    ; NO LOOP 
	;STA $4100   ; PUSH 00  to $4100
	;JMP play_bgm
	;bgm_loop:
	;LDA #%00000001    ; LOOP ENABLE
	;STA $4100   ; PUSH 01(LOOP) to $4100


play_bgm:
   
    ; A = requested BGM track (00+)
    ;CMP nea_last_bgm
    ;BEQ clear_music              ; same as last -> don't spam
    ...
    STA $4104
    LDA ram_music_1
    STA $4105              ; NEA BGM track = A 

clear_music:
    ; Clear music request (matches Ice Climber engine behavior)
    LDA #$00
    STA ram_music_1
  nea_rts:  
    RTS
